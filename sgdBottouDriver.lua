...
function sgdBottouDriver(opfunc, config, nSamples, initialX, tolX, tolF, maxEpochs, verbose)
   ...
   repeat 
      ...
      local converged = 
         (tolX ~= nil and nEpochs > 1 and torch.norm(x - lastX) < tolX) or
         (tolF ~= nil and nEpochs > 1 and math.abs(avgLoss - lastAvgLoss) < tolF) or
         (nEpochs >= maxEpochs)
